      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 h-16 flex items-center justify-between">
        <Link to="/" className="flex min-w-0 items-center" aria-label="GoChrome home">
          <img
            src={logo}
            alt="GoChrome"
            className="h-9 w-40 object-contain object-left mix-blend-multiply dark:invert sm:h-10 sm:w-48"
          />
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {nav.map((n) => (
            <Link
              key={n.to}
              to={n.to}
              className={`text-sm transition-colors hover:text-foreground ${path === n.to ? "text-foreground" : "text-muted-foreground"}`}
            >
              {n.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <span className="text-xs font-medium whitespace-nowrap">Track Order</span>
          </Link>
          <Link
            to="/cart"
            className="h-9 px-3 rounded-full hover:bg-accent transition-colors flex items-center gap-2 text-sm"
          >
          ...
          <button
            ...
            className="lg:hidden h-9 w-9 grid place-items-center rounded-full hover:bg-accent"
          >
