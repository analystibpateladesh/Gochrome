  ...
  if (!items.length) {
    return (
      <section className="mx-auto max-w-3xl px-6 py-32 text-center">
        ...
        <Link
          to="/chrome"
          className="mt-8 inline-block px-7 py-3.5 rounded-full bg-primary text-primary-foreground text-sm font-medium"
        >
          Shop GoChrome
        </Link>
      </section>
  ...
  return (
    <section className="mx-auto max-w-5xl px-6 py-20">
        ...
        {items.map((i) => (
          <div key={i.id} className="flex gap-5 card-soft p-4">
            ...
            <div className="text-right">
              ...
              <button
                onClick={() => remove(i.id)}
                className="mt-2 text-muted-foreground hover:text-destructive text-xs inline-flex items-center gap-1"
              >
      ...
      <div className="mt-10 card-soft p-6">
        ...
        <div className="flex justify-between text-lg font-medium mt-4 pt-4 border-t border-border">
          <span>Total</span>
          <span>₹{total.toLocaleString("en-IN")}</span>
        </div>
        <Link
          to="/checkout"
          className="mt-6 block text-center px-7 py-3.5 rounded-full bg-primary text-primary-foreground font-medium hover:opacity-90"
        >
          Checkout
        </Link>
      </div>
